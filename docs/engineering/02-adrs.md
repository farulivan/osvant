...
- **Consequences:**
  ...
  - **Automatic content detection is off** (`source(none)` + an explicit `@source` over `src/`). Left on, Tailwind scans every non-ignored file as plain text — including 9,000 lines of normative docs in which "block", "grid", "static" and "fixed" are ordinary English words, each of which would mint a utility nobody asked for.
