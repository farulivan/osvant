...
- **Flagged for PR 8 (OSV-31), not fixed here.** LCP is 2.6s against the 2.5s budget on `/` and the fever PDP (perf 97, a11y 100, best-practices 100, SEO 100, CLS ≤0.001, TBT ≤20ms; `/the-house` passes at 2.4s). The photography is **not** the cause — measured, not assumed: FCP is 1.3s, and stripping the preloader from the built HTML moved LCP by 0ms. The LCP element on `/` is the hero `h1`, which `headline-reveal.ts` holds at `opacity: 0` via `gsap.from` until the module mounts and the stagger completes. The signature reveal is what costs the 1.3s. Fixing it means changing `M §4.2`, which is LAW — escalating per guide §7 rather than deciding unilaterally.
- **Asset gap:** the craft grid borrows ingredient macros because the set has no process photography — it wants a bench, an organ and a batch sheet. `campaign/skin-textures.jpg` and `macro/mint-leaf.jpg` are in the repo but deliberately unimported: Astro emits an imported asset whether or not anything renders it, so an unused import ships its full-resolution original (skin-textures alone was 5.6MB).
- Bundle 69.89KB gzip against the 350KB budget; deploy 3.1MB; `pnpm verify` green (146 tests), copy and asset guards clean.
