...
> _provocation, bottled._ — a fictional luxury fragrance house, built as a portfolio showcase of animation-first front-end engineering.
...
**In build — milestone M1 (Foundation).** Build sequence lives at `docs/engineering/00-implementation-guide.md` §5; per-PR history in `docs/engineering/worklog.md`.

## Development

Node 24 (`.nvmrc`) + [pnpm](https://pnpm.io) (version pinned via `packageManager`).
...
pnpm verify     # format → lint → typecheck → unit tests (run before every push)
```
...
The process artifacts stay in-repo on purpose — the docs, the RFC's question-driven spec hardening, the ADR trail (including a mid-flight re-scope, ADR-009–012), and the commit history _are_ part of the portfolio. Agent ground rules live in [`AGENTS.md`](AGENTS.md).
